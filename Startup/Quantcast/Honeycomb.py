...
class Node:
    ...
    def assignNeighbors(self, direction, node):
...
def bfs(start, word):
    ...
    while queue:
        ...
        if node and node not in visited:
            next_letter = word[letter_index]
            if node.getLetter() == next_letter:
                visited.add(node)
                # If last letter was just found, return True
                if letter_index+1 > word_length-1:
                    return True
                queue.extend([(neighbor, letter_index+1) for neighbor in node.getNeighbors()])
                next_letter = word[letter_index]
    ...
    for triangle_index in range(6):
        for layer in honeycomb_layers[1:]:
            ...
            for node in layer[start:end]:
                ...
                node.assignNeighbors(first_dxn, inner_node)
                node.assignNeighbors(counterclockwise(first_dxn), next_node)
                next_node.assignNeighbors(clockwise(first_dxn), inner_node)
